...
public class EmployeeStreamsTest {
    ...
    public void findAll_nameLengthAtMost5_sortBySalary() {
        List<Employee> employees = allEmployees.stream()
                ...
                .sorted((emp, emp2) -> Double.compare(emp.getSalary(), emp2.getSalary()))
                .collect(toList());
        dump(employees);
    ...
    public void findAll_salaryAtLeast50000_sortByHireDate() {
        List<Employee> employees = allEmployees.stream()
                ...
                //.sorted((emp, emp2) -> emp.getHireDate().compareTo(emp2.getHireDate()))
    ...
    public void count_salaryAtLeast75000() {
        long employees = allEmployees.stream()
    ...
    public void threeHighestPaid_sortByName() {
        List<Employee> employees = allEmployees.stream()
//                .sorted((emp, emp2) -> emp2.getSalary().compareTo(emp.getSalary()))
    ...
    public void findAllNames_hired2010Later() {
        List<String> employees = allEmployees.stream()
    ...
    public void payAll_salaryAtMost50000_lowestPaidFirst() {
        allEmployees.stream()
                .filter(emp -> emp.getSalary() <= 50_000.0)
                .sorted((emp,emp2) -> emp.getSalary().compareTo(emp2.getSalary()))
    ...
    public void giveRaise_salaryLessThan50000_hiredAfter2000_lowestPaidFirst() {
        List<Employee> employees = allEmployees.stream()
                .filter(employee -> employee.getHireDate().getYear() >= 2000 && employee.getSalary() < 50_000.0)
                .sorted(comparing(Employee::getSalary))
                .peek(employee -> employee.setSalary(100_000.0))
                .collect(toList());
        dump(employees);
    }
    ...
    public void allEmployeesHired2000_orLater() {
        boolean hasYoungin = allEmployees.stream()
                .allMatch(employee -> employee.getHireDate().getYear() >= 2000);
        System.out.println(hasYoungin);
    }
    ...
    public void averageSalary_hired2000_orLater() {
        double result = allEmployees.stream()
                .filter(employee -> employee.getHireDate().getYear() >= 2000)
    ...
    public void twoGroups_atLeast50K_lessThan50K() {
        Map<Boolean, List<Employee>> employeeMap = allEmployees.stream()
                .collect(Collectors.partitioningBy(employee -> employee.getSalary() >= 50_000.0));
        dump(employeeMap);
    }
    ...
    * */
    @Test
    public void groupingByYearOfHireCount() {
        Map<Integer, Long> yearCountMap = allEmployees.stream()
                .collect(Collectors.groupingBy(employee -> employee.getHireDate().getYear(), counting()));
        dump(yearCountMap);
    }
    /**
